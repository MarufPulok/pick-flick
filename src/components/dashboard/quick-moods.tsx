...
const MOODS: MoodOption[] = [
  ...
  },
  {
    id: 'thriller',
    label: 'Thrills',
    icon: <Skull className="w-4 h-4 sm:w-5 sm:h-5" />,
      ...
      <div className="flex gap-2 sm:gap-3 overflow-x-auto pb-2 -mx-1 px-1 scrollbar-hide">
        {MOODS.map((mood) => (
          <button
            ...
            className={`flex items-center gap-2 px-4 sm:px-5 py-2.5 sm:py-3 rounded-full bg-gradient-to-r ${mood.gradient} text-white font-medium text-sm sm:text-base whitespace-nowrap hover:scale-105 hover:shadow-lg transition-all duration-200 active:scale-95 min-h-[44px] shrink-0`}
          >
            {mood.icon}
            {mood.label}
          </button>
